                    ...
                    <el-col :xs='24' :sm="24" :md="12" :lg="12" :xl="12">
                        <div class="overview-wrapper">
                            ...
                            <div class="overview-item-base" v-loading="loadingTransaction">
                                <div class="block-item font-color-2e384d" v-for="item in transactionList" :key='item.txId'>
                                    <div class="block-amount">
                                        <p class="trans-hash" :title="`${item.txId}`">
                                            <i class="wbs-icon-copy font-12" @click="copyNodeIdKey(item.txId)" title="复制"></i>
                                            <router-link :to="{'path': 'transactionInfo', 'query': {blockNumber: item.txId}}" class="node-ip">
                                                {{item.txId}}
                                            </router-link>
                                        </p>
                                    ...
                                    <p class="color-8798AD text-right">{{item.transTimestamp}}</p>
                                </div>
                            </div>
                        </div>
                    </el-col>
...
export default {
    ...
    methods: {
        ...
        getChart: function () {
            ...
            let channelId = localStorage.getItem("channelId");
            getTransactionDaily(channelId)
                .then(res => {
                    this.loadingCharts = false;
                    if (res.data.code === 0) {
                        for (let i = 0; i < res.data.data.length; i++) {
                            this.chartStatistics.date.push(res.data.data[i].day);
                            if(res.data.data[i].transCount){
                                this.chartStatistics.dataArr.push(
                                    res.data.data[i].transCount
                                );
                            }else{
                                this.chartStatistics.dataArr.push(0)
                            }
                        }
