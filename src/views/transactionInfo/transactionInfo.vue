                ...
                <el-table  :data="transactionList" class="block-table-content" :row-key="getRowKeys" :expand-row-keys="expands" v-loading="loading" @row-click="clickTable" ref="refTable">
                    <el-table-column type="expand" align="center">
                        <template slot-scope="scope">
                            <v-transaction-detail :transHash="scope.row.txId"></v-transaction-detail>
                        </template>
                    </el-table-column>
                    <el-table-column prop="txId" label="交易哈希" align="center" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>
                                <i class="wbs-icon-copy font-12 copy-key" @click="copyPubilcKey(scope.row['txId'])" title="复制哈希"></i>
                                {{scope.row['txId']}}
                            </span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="blockNumber" label="块高" width="180" align="center" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>{{scope.row['blockNumber']}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="envelopeType" label="交易类型" width="200" align="center" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>{{scope.row['envelopeType']}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="transTimestamp" label="创建时间" width="180" :show-overflow-tooltip="true" align="center">
                        <template slot-scope="scope">
                            <span>{{scope.row['transTimestamp']}}</span>
                        </template>
                    </el-table-column>
                </el-table>
