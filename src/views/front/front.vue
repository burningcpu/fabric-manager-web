                    ...
                    <el-table-column v-for="head in frontHead" :label="head.name" :key="head.enName" show-overflow-tooltip>
                        <template slot-scope="scope">
                            <span v-if='head.enName === "frontIp"'>
                                <span class="link" @click=link(scope.row)>{{scope.row[head.enName]}}</span>
                                <!-- <router-link :to="{'path': 'hostDetail', 'query': {nodeIp: scope.row.frontIp, frontId: scope.row.frontId}}" class="link">{{scope.row[head.enName]}}</router-link> -->
                            </span>
...
import { getFrontList , addnodes, deleteFront, getPeerList, getConsensusNodeId } from "@/util/api";
