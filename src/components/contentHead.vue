    ...
    <div class="content-head-wrapper">
        ...
        <div class="content-head-network">
            ...
            <el-popover placement="bottom" width="120" min-width="50px" trigger="click">
                <ul class="group-item">
                    <li class="group-item-list" v-for='item in channelList' :key='item.channelId' @click='changeGroup(item)'>{{item.channelName}}</li>
                </ul>
                <span slot="reference" class="contant-head-name" style="color: #fff" @click='checkGroup'>通道: {{channelName || '-'}}</span>
            </el-popover>
...
import { loginOut, getChannelList } from "@/util/api";
...
export default {
    ...
    mounted: function () {
        if (localStorage.getItem("channelName")) {
            this.channelName = localStorage.getItem("groupName");
        }
    ...
    methods: {
            ...
            getChannelList().then(res => {
                if (res.data.code === 0) {
                    ...
                    if (type && res.data.data && res.data.data.length) {
                        this.channelName = res.data.data[0].channelName;
                        localStorage.setItem("channelName", res.data.data[0].channelName)
                        localStorage.setItem("channelId", res.data.data[0].channelId)
                    } else if (res.data.data && res.data.data.length && !localStorage.getItem("channelName")) {
                        this.channelName = res.data.data[0].channelName;
                        localStorage.setItem("channelName", res.data.data[0].channelName)
                        localStorage.setItem("channelId", res.data.data[0].channelId)
                    } else if (res.data.data && res.data.data.length && localStorage.getItem("channelName")) {
        ...
        changeGroup: function (val) {
            ...
            localStorage.setItem("channelName", val.channelName);
            localStorage.setItem("channelId", val.channelId);
            this.$emit('changGroup', val.channelId);
            this.dialogShow = true;
        },
